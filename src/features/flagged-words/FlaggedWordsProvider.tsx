...
export function FlaggedWordsProvider({ children }: { children: ReactNode }) {
  const [settings, setSettings] = useState<FlaggedWordsSettings>(() => readFlaggedWordsSettings());
  ...
  const findMatches = useCallback((name: string) => {
    return findFlaggedWords(name, settings.words);
  }, [settings.words]);

  const findTransactionMatches = useCallback((transactions: Array<{ name: string }>): FlaggedWordMatch[] => {
    return transactions
      .map((transaction) => ({
