...
export function parseShortcut(value: string): ShortcutBinding | null {
  ...
  return {
    key: normalizeShortcutParts({
      ...
      ctrl:
        normalizedModifiers.has("ctrl") || normalizedModifiers.has("control"),
      alt: normalizedModifiers.has("alt") || normalizedModifiers.has("option"),
      shift: normalizedModifiers.has("shift"),
      meta:
        normalizedModifiers.has("meta") ||
        normalizedModifiers.has("cmd") ||
        normalizedModifiers.has("command"),
    }),
  };
}
...
export function validateShortcut(
  ...
  if (reservedShortcuts.has(binding.key)) {
    return {
      ok: false,
      message: `${displayShortcut(binding)} is reserved by browsers, operating systems, or assistive technology.`,
    };
  }
  ...
  if (isSingleCharacterShortcut(binding) && scope === "global") {
    return {
      ok: false,
      message: "Global shortcuts must include a modifier key.",
    };
  }
