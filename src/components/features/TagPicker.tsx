...
export function TagChip({ tag, onRemove, className }: { tag: Pick<Tag, 'id' | 'name' | 'type' | 'color'>; onRemove?: () => void; className?: string }) {
...
export const TagPicker = forwardRef<HTMLButtonElement, TagPickerProps>(({
  ...
  const toggleTag = (tagId: string) => {
    onChange(selectedIds.has(tagId) ? value.filter((id) => id !== tagId) : [...value, tagId]);
  };
  ...
  const handleCreate = async () => {
    ...
    const existing = activeTags.find(
      (tag) => tag.type === newType && normalizeTagName(tag.name).toLowerCase() === name.toLowerCase(),
    );
    if (existing) {
      const latestValue = valueRef.current;
      if (!latestValue.includes(existing.id)) onChange([...latestValue, existing.id]);
      setNewName('');
      return;
    }
        ...
        >
            ...
            {activeTags.map((tag) => {
              ...
              return (
                ...
                >
                  ...
                  {selected && <Check className="h-3.5 w-3.5 text-green-400" />}
                </button>
              );
            })}
          </div>
          <div className="mt-2 border-t border-border pt-2">
            <div className="flex gap-2">
              ...
              />
              <Button type="button" size="sm" onClick={() => void handleCreate()} loading={creating} disabled={disabled} className="h-8 px-2">
            ...
            <SimpleSelect
              ...
              options={TAG_TYPES.map((type) => ({ value: type, label: tagTypeLabel(type) }))}
              disabled={disabled || creating}
