  ...
  return (
    <>
      <button
        ...
        className={cn(
          'fixed bottom-20 right-4 z-40 inline-flex h-12 w-12 items-center justify-center rounded-full bg-primary text-black shadow-[rgba(0,0,0,0.5)_0px_8px_24px] transition-transform hover:scale-105 md:bottom-5',
          open && 'hidden',
        )}
      ...
      {open && (
        <aside className="fixed inset-0 z-50 flex flex-col bg-background shadow-[rgba(0,0,0,0.5)_0px_8px_24px] md:inset-y-4 md:right-4 md:left-auto md:w-[28rem] md:rounded-lg">
          <div className="flex h-14 shrink-0 items-center gap-2 border-b border-border/50 bg-card px-4 md:rounded-t-lg">
            ...
            <button
              ...
              className="lf-icon-button h-8 w-8"
          ...
          <div className="flex-1 space-y-3 overflow-y-auto px-4 py-3">
            ...
            {messages.map((message) => (
              ...
              >
                ...
                {message.actions && message.actions.length > 0 && (
                  <div className="mt-2 space-y-1 border-t border-border pt-2 text-xs">
                    {message.actions.map((action, index) => (
                      <details key={`${action.type}-${index}`} className="rounded-lg bg-secondary p-2">
                ...
                <div className="mt-2 max-h-48 space-y-2 overflow-auto rounded-lg bg-secondary p-2">
            ...
            {streamState && (
              <div className="mr-8 rounded-lg bg-card px-3 py-2 text-sm text-foreground">
                ...
                {streamState.actions.length > 0 && (
                  <div className="mt-2 space-y-1 border-t border-border pt-2 text-xs">
                    {streamState.actions.map((action, index) => (
                      <details key={`${action.type}-${index}`} open className="rounded-lg bg-secondary p-2">
                        <summary className="flex cursor-pointer items-start gap-1.5">
                ...
                {streamState.responseDraft && (
                  <details className="mt-2 border-t border-border pt-2 text-xs">
                    ...
                    <pre className="mt-2 max-h-48 overflow-auto rounded-lg bg-secondary p-2 text-[11px] leading-relaxed">
                      {streamState.responseDraft}
                    </pre>
                  </details>
                )}
          ...
          <div className="border-t border-border/50 bg-card p-3 md:rounded-b-lg">
            <form
            ...
            >
              <textarea
                ...
                className="h-24 min-w-0 flex-1 resize-none rounded-lg border-0 bg-input px-2 py-2 text-sm text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              />
