...
export function saveAICorrection(data: SaveAICorrectionData): AICorrection {
  ...
  if (existing) {
    ...
    return db
      .prepare("SELECT * FROM ai_corrections WHERE id = ?")
      .get(existing.id) as AICorrection;
  }
  ...
  return db
    .prepare("SELECT * FROM ai_corrections WHERE id = ?")
    .get(id) as AICorrection;
}

export function getAICorrection(
...
export function getAICorrections(): AICorrection[] {
  const db = getDb();
  return db
    .prepare("SELECT * FROM ai_corrections ORDER BY created_at DESC")
    .all() as AICorrection[];
}
